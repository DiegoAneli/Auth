...
export default async function register(req: NextApiRequest, res: NextApiResponse) {
  ...
  let role = 'admin'; // Ruolo di default per i nuovi utenti

  // Verifica il token di invito (se esiste) e assegna un ruolo specifico
  if (token) {
    try {
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      const { projectId, role: tokenRole } = decoded;

      // Aggiorna il ruolo in base al token di invito
      if (tokenRole) {
        role = tokenRole;
      }

      // Aggiungi l'utente come collaboratore nel progetto specifico
      await db.collection('users').updateOne(
        { 'projects._id': new ObjectId(projectId) },
        { $addToSet: { 'projects.$.collaborators': { email: lowercasedEmail, role } } }
      );
...
export default async function register(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Ruolo di default per i nuovi utenti è admin
  const role = 'admin';
  ...
  const newUser = {
    ...
    isVerified: false,
    projects: [],
    role, // Ruolo utente impostato a 'admin'
  };
